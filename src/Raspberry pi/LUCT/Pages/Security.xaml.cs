...
namespace LUCT
{
    public sealed partial class SecurityTestPage : Page
    {
        ...
        private async void speechMediaElement_Loaded(object sender, RoutedEventArgs e)
        {
            if (speech == null)
            {
                ...
                status.Text = SpeechContants.InitialGreetingMessage;
            }
            else
            {
        ...
        private async Task DoorbellPressed()
        {
            ...
            if (webcam.IsInitialized() && initializedOxford)
            {
                ...
                else
                {
                    ...
                    status.Text = "Image not recognized";
                }
            }
            else
            {
                if (!webcam.IsInitialized())
                {
                    ...
                    status.Text = "Camera failed to initialize";
        ...
        private async void UnlockDoor(string visitorName)
        {
            ...
            status.Text = SpeechContants.GeneralGreetigMessage(visitorName);
        ...
        /// </summary>
        private async void backPageButton_Click(object sender, RoutedEventArgs e)
        {
            await webcam.StopCameraPreview();
            // Exit app
        ...
        private void banButton_Tapped(object sender, Windows.UI.Xaml.Input.TappedRoutedEventArgs e)
        {
            Frame.Navigate(typeof(BanPage));
        }
    }
}